function batch_Bayesian(Batch_id, num_subj_per_batch, step_number, version, fid)
if nargin <= 4
    fid = 1;
end
...
for subj_cnt = num_subj_per_batch : -1 : 1        % Reverse loop, elimitnate need to pre-allocate space for many variables
    ...
    for Bayesian_version = version
        T_algo_start = tic;
        eval(sprintf('kontsevich%d(subj_cnt) = BayesianAdaptiveEstimation(params, Bayesian_version);', Bayesian_version));
        T_algo_end = toc(T_algo_start);
        fprintf(fid, '\t\tKontsevich-Tyler version %d: %02d:%02d.%03d.\n\t\t\t', Bayesian_version, fix(T_algo_end/60) , fix(mod(T_algo_end,60)), fix(mod(T_algo_end*1000,1000)) );

        eval(sprintf('%s\\n'', %s, %s * 100, %s );', txt_fprintf, '''''', ...
            sprintf('kontsevich%d(subj_cnt).a_estim(end)', Bayesian_version), ...
            sprintf('kontsevich%d(subj_cnt).rel_err(end)', Bayesian_version)));
    end 
    %% %%%%%%%%%%%%%%%%%%%%%%%
