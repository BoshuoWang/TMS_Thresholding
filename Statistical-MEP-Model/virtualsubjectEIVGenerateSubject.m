function subject_parameters = virtualsubjectEIVGenerateSubject()
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Simulation Test Bed
...
%
%   The underlying statistical distributions are described in the paper:
%   S. M. Goetz, S. M. M. Alavi, Z. -D. Deng and A. V. Peterchev,
%   "Statistical Model of Motor-Evoked Potentials," in IEEE Transactions on
%   Neural Systems and Rehabilitation Engineering, vol. 27, no. 8, pp.
%   1539-1545, Aug. 2019, doi: 10.1109/TNSRE.2019.2926543.

%   subject_parameters = [ <p1=add_var_y_mu>, <p2>, <p3>, <p4>, <p5>,...
%                        <mult_var_y_mu>, <mult_var_y_sigma>, ...
%                        <add_var_x_mu>, <add_var_x_sigma>, ...
%                        <add_var_y_sigma>, <k> ];
%
...
while ~(valid_IO)
    % Multidimensional Gaussian including correlation based on cholesky decomposition of covariance matrix
    rand_normal_vec = mvnrnd(aposteriori_mu, aposteriori_covmtrx, 1);	% generate one parameter set for next run
    subject_parameters(1:2) = rand_normal_vec(1:2);
    subject_parameters(3:5) = 10.^(rand_normal_vec(3:5));              	% p3 to p5 log-normal
   
    % Parameters for multiplicative y and additive x variability
    subject_parameters(6:7) = 10.^(var_mu + var_sigma .* randn([1,2]));
   
    % Parameters for additive y variability (not correlated):
    subject_parameters(8) = 1.55e-6 * 1.5^(randn(1));     % sigma of GEV
    subject_parameters(9) = 0.14 * 2^(randn(1));          % k of GEV
    ...
    y_vec = virtstimulate(stim_vec, subject_parameters);
    if (any(y_vec < 30e-6)) && (any(y_vec >  1e-3)) && ...  % Cover range of no response to saturation: from less than 30 muV to more than 1 mV)
       (all(y_vec >  1e-7)) && (all(y_vec < 30e-3))         % No outliers: less than 0.1 muV and more than 30 mV)
        valid_IO = true;
    end
end
