function batch_MLE(Batch_id, num_subj_per_batch, step_number, version)
T_batch_start = tic;
...
for subj_cnt = num_subj_per_batch : -1 : 1        % Reverse loop, elimitnate need to pre-allocate space for many variables
    ...
    for MLE_ver = version
        ...
        fprintf('\t\tMLE version %d: %02d:%02d.%03d.\n\t\t\t', MLE_ver, fix(T_algo_end/60) , fix(mod(T_algo_end,60)), fix(mod(T_algo_end*1000,1000)) );
        
        eval(sprintf('%s\\n'', %s, %s * 100, %s );', txt_fprintf, '''MLE ''', sprintf('MLE%d(subj_cnt).MLE_t(end)', MLE_ver), sprintf('MLE%d(subj_cnt).MLE_rel_err(end)', MLE_ver)));
        fprintf('\t\t\t');
        eval(sprintf('%s\\n'', %s, %s * 100, %s );', txt_fprintf, '''MAP ''', sprintf('MLE%d(subj_cnt).MAP_t(end)', MLE_ver), sprintf('MLE%d(subj_cnt).MAP_rel_err(end)', MLE_ver)));
    end
